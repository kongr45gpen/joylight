    ...
    EffectNodeDefinition {
        ...
        processor: Box::new(move |inputs: &NodeDataset, _, _| {
            ...
            for fixture in selection.fixtures() {
                debug!("iterating over fixture {} for parameter {}",  fixture.uuid(), named_parameter);
                ...
                fixture.write(|fxt| {
                    fxt.get_parameter_by_name(named_parameter.as_str())
                        ...
                        .and_then(|index| {
                            ...
                            fxt.set_parameter(index, parameter_value::ParameterValue::Number(paket.iter().copied().collect()))
                        })
